...
- `ATLETA` (terceiro perfil) — **em análise, tendência a NÃO fazer**. Presidente + Gustavo acham que login de atleta complica/pesa o sistema sem necessidade (muito mais usuários, banco bem maior, custo de hospedagem podendo ~dobrar). Se voltar, avaliar alternativa **sem login** (consulta pública por CPF). Ver `docs/MODULO-COMPETICOES.md` §7.
...
- **Dados de teste:** `scripts/seed_teste.sql` popula 2 clubes + 7 atletas (status/idades variados, inclui menores) direto no Postgres pra testar os painéis. Rodar: `docker exec -i fht_postgres psql -U fht_user -d fht_db < scripts/seed_teste.sql`.
